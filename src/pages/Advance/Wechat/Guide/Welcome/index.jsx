...
export default class Welcome extends React.Component {
        ...
        switch (file.status) {
            case 'done':
                if (0 === file.response.errcode) {
                    message.success(intl.get(MODULE, 17)/*_i18n:上传成功*/);
                    this.updateImg(imgKey);
                } else {
                    message.error(intl.get(MODULE, 18)/*_i18n:上传失败，请检查图片格式、大小是否符合要求*/);
                }
                break;
            case 'error':
                ...
                message.error(intl.get(MODULE, 19)/*_i18n:上传失败，请检查图片格式、大小是否符合要求*/);
                break;
        }
    }
    ...
    beforeUpload = (file) => {
        ...
        message.error(intl.get(MODULE, 20)/*_i18n:只支持.jpg .jpeg .png格式的图片*/);
        return false;
    }
    ...
    onChange = (name, value) => {
        let check = {
            logo: {
                func: checkStr(value, { who: intl.get(MODULE, 21)/*_i18n:logo信息*/, min: 1, max: 15 })
            },
            welcome: {
                func: checkStr(value, { who: intl.get(MODULE, 22)/*_i18n:欢迎信息*/, min: 1, max: 30 })
            },
            btnStr: {
                func: checkStr(value, { who: intl.get(MODULE, 23)/*_i18n:提示文字*/, min: 1, max: 15 })
            },
            statement: {
                func: checkStr(value, { who: intl.get(MODULE, 24)/*_i18n:版权声明*/, min: 1, max: 30 })
            },
        };
                    ...
                    <Form className='setup-form'>
                        <div className='file-upload'>
                            <Upload
                                ...
                                data={{ opcode: '0x2086' }}
                                action={__BASEAPI__}
                                uploadTitle={intl.get(MODULE, 25)/*_i18n:上传Logo图*/}
                                multiple={false}
                                headers={{
                                    'XSRF-TOKEN': get(),
                                }}
                            ...
                            <Upload
                                ...
                                uploadTitle={intl.get(MODULE, 26)/*_i18n:上传背景图*/}
                                beforeUpload={this.beforeUpload}
                            >
                        ...
                        <label>{intl.get(MODULE, 27)/*_i18n:Logo信息*/}</label>
