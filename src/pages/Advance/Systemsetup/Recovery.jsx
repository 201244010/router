...
export default class Recovery extends React.Component{
    constructor(props){
        ...
        this.columns = [{
            ...
            title: intl.get(MODULE, 13)/*_i18n:接入方式*/,
            width: 240,
            render: (_, record) => {
                const online = record.online;
                return <div><i className={'dot ' + (online ? '' : 'offline')}></i><span style={{fontSize: 12}}>{online ? intl.get(MODULE, 14) : intl.get(MODULE, 15)}</span></div>                                
            }
        }, {
            title: intl.get(MODULE, 16)/*_i18n:接入方式*/,
            dataIndex: 'operation',
            width: 336,
            render: (_, record) => {
                const online = record.online;
    ...
    fetchRouter = async () => {
        ...
        const {errcode, data} = resp;
        if (errcode !== 0) {
            message.warning(intl.get(MODULE, 18))
        }
        const routerList = [];
        ...
        return (
             <SubLayout className="settings">
                        <div className="system-recovery">
                            <label className="recovery-title">{intl.get(MODULE, 19)}</label>
                            <Button style={{height: 32}} disabled={disabled} type="primary" onClick={this.showModal}>{intl.get(MODULE, 20)}</Button>
                        </div>
                        <div className="static-table">
                            <Table
                                ...
                                style={{ minHeight: 360 }}
                                pagination={false}
                                locale={{ emptyText: intl.get(MODULE, 21)/*_i18n:暂无设备*/, filterConfirm: intl.get(MODULE, 9)/*_i18n:确定*/, filterReset: intl.get(MODULE, 29)/*_i18n:重置*/ }}
                                />
                        </div>
                        {loadingActive &&
                            <CustomProgress
                        ...
                        <Modal
                            ...
                            footer={
                                [
                                    <Button onClick={this.recoveryCancel}>{intl.get(MODULE, 4)}</Button>,
                                    <Button className="reboot-confirm" onClick={() => this.reset(this.onlineDev, 1)}>{intl.get(MODULE, 9)}</Button>
                                ]
                            }
                        >
                            <span className="reboot-modal-content">{intl.get(MODULE, 22, {duration})}</span>
                        </Modal>
                </SubLayout>
        );
    }
