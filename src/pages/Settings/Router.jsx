        ...
        this.columns = [{
            ...
            render: (name, record) => {
                return <div className="sub-router-set">
                    ...
                    <ul>
                        <li>{name}</li>
                        <li>{intl.get(MODULE, 1)/*_i18n:运行时长：*/}{record.online ? record.uptime : '--'} </li>
                    </ul>
                    </div>
                </div>
            }
            ...
            render: (rssi, record) => {
                if (record.online) {
                    return <div>
                        <i className={'dot ' + (intl.get(MODULE, 7)/*_i18n:较差*/ == rssi ? 'warning' : '')}></i>
                        <span style={{fontSize: 12}}>{rssi}</span>
                    </div>
                } else {
                    return <div>
                            <i className='dot offline'></i>
                            <span style={{fontSize: 12, color: '#ADB1B9'}}>{intl.get(MODULE, 8)/*_i18n:已离线*/}</span>
                    </div>
                }
            }
        }, {
                ...
                return  <Popconfirm
                            title= {
                                <div className="pop-content">
                                    <label>{intl.get(MODULE, 10)/*_i18n:你确定要将此设备从网络中移除？*/}</label>
                                    <p>{intl.get(MODULE, 11)/*_i18n:移除之后，子路由将自动重置为出厂状态，耗时约2分钟，期间请勿断电*/}</p>
                                </div>
                            }
                            ...
                            <span style={{fontSize: 14, color: '#6174F1', cursor: 'pointer'}}>{intl.get(MODULE, 12)/*_i18n:移除*/}</span>
                        </Popconfirm>
                }
        }];
    }
        ...
        let routerList = router.filter((router) => router.role === '0').map(router => {
            if (router.role === '0' ) {
                ...
                if (connMode.wired === 1) {
                    ...
                    rssi = router.rssi >= 20 ? intl.get(MODULE, 16)/*_i18n:较好*/ : intl.get(MODULE, 17)/*_i18n:较差*/;
                }
                let mode = '';
                connMode.w_2g && (mode = mode + '/2.4G');
                connMode.w_5g && (mode = mode + '/5G');
                connMode.wired && (mode = mode + `/${intl.get(MODULE, 18)/*_i18n:有线*/}`);
            ...
            <SubLayout className="settings">
                <div className="static-table" style={{marginTop:24}}>
                    <Table
                        ...
                        style={{ minHeight: 360 }}
                        pagination={false}
                        locale={{ emptyText: intl.get(MODULE, 21)/*_i18n:暂无设备*/, filterConfirm: intl.get(MODULE, 15)/*_i18n:请求失败*/, filterReset: intl.get(MODULE, 29)/*_i18n:undefined*/ }}
                        />
                </div>    
            </SubLayout>
        )
