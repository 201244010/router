        ...
        if (0 === showList.length && !searchFinish) {       //搜索进行中，且设备列表为空
            return (
                <React.Fragment>
                    ...
                    <h3 className='bigTitle'>{intl.get(MODULE, 2)}</h3>
                </React.Fragment>
            );
        }
        ...
        if (0 === showList.length && searchFinish) {        //搜索完成，且设备列表为空
            return (
                <React.Fragment>
                    ...
                    <p className='noFindTip'>{intl.get(MODULE, 3)}</p>
                    <div className='warning'>
                        <p className='warningTip' style={{marginBottom: 8}}><CustomIcon size={12} color='#FB8632' type="hint" style={{marginRight: 4}}/>{intl.get(MODULE, 4)}</p>
                        <p className='warningTip'><CustomIcon size={12} color='#FB8632' type="hint" style={{marginRight: 4}}/>{intl.get(MODULE, 5)}</p>
                    </div>
                    <Button type="primary" className="settingButton" onClick={this.props.reSearch}>{intl.get(MODULE, 6)}</Button>
                </React.Fragment>
            );
        }
            ...
            return (
                <React.Fragment>
                    <div className="settingHeader">
                        ...
                        <span className='smallTitle'>{intl.get(MODULE, 7)}</span>
                    </div>
                    <div className="body">
                    {showList}
                    </div>
                    <Button type="primary" className="settingButton" disabled={disabled} onClick={this.props.setSubRouter}>{intl.get(MODULE, 8)}</Button>
                </React.Fragment>
            );
        }
            ...
            return (
                <React.Fragment>
                    <div className="settingHeader">
                        ...
                        <span className='smallTitle' style={{verticalAlign: 'middle'}}>{intl.get(MODULE, 9)}</span>
                    </div>
                    <div className="body">
                    {showList}
                    </div>
                    <Button type="primary" className="settingButton" disabled={disabled} onClick={this.props.setSubRouter}>{intl.get(MODULE, 10)}</Button>
                    <p className="research" onClick={this.props.reSearch}>{intl.get(MODULE, 11)}</p>
                </React.Fragment>
            );
        }
    }
        ...
        return (
            <React.Fragment>
                <div className="settingHeader">
                    ...
                    <span className='smallTitle'>{intl.get(MODULE, 12)}</span>
                </div>
                <div className="body">
                {showList}
                </div>
    ...
    render() {
        ...
        if ('allOK' === statement && 0 < showList.length && !haveGiveUp) {      //设置都成功的情况
            return (
                ...
                <div className="settingHeader">
                    <CustomIcon size={24} color='#4EC53F' type="succeed" style={{marginRight: 8}}/>
                    <span className='smallTitle' style={{verticalAlign: 'middle'}}>{intl.get(MODULE, 13)}</span>
                </div>
                <div className="body">
                {showList}
                </div>
                <Button type="primary" className="settingButton" disabled={false} onClick={this.props.next}>{intl.get(MODULE, 14)}</Button>
            </React.Fragment>
            );
        }
        ...
        if ('allOK' === statement && 0 < showList.length && haveGiveUp) {       //忽略失败设备后都成功的情况
            return (
                ...
                <Button type="primary" className="settingButton" disabled={false} onClick={this.props.next}>{intl.get(MODULE, 15)}</Button>
            </React.Fragment>
            );
        }
        ...
        if ('haveFailed' === statement) {       //存在设置失败的设备，包括忽略某台设备后还存在其他失败的设备的情况
            return (
                ...
                <div className="haveFailed">
                    <p>{intl.get(MODULE, 16)}</p>
                </div>
                <div className="body">
                {showList}
                </div>
                <Button type="primary" className="settingButton" disabled={true} onClick={this.props.next}>{intl.get(MODULE, 17)}</Button>
            </React.Fragment>
            );
        }
        ...
        if (haveGiveUp && 0 === showList.length) {  //失败设备都忽略后，没有设备的情况（原本设置都失败）
            return (
                ...
                <div className="settingHeader">
                    ...
                    <span className='smallTitle' style={{verticalAlign: 'middle'}}>{intl.get(MODULE, 18)}</span>
                </div>
                <div className="body">
                    <CustomIcon size={140} color='#D7D8DC' type="noroute"/>  
                </div>
                <Button type="primary" className="settingButton" disabled={false} onClick={this.props.goHome}>{intl.get(MODULE, 19)}</Button>
            </React.Fragment>
            );
        } 
    }
