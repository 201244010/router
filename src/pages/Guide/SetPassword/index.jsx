...
export default class SetPassword extends React.Component {
	...
	onChange = (name, value) => {
		const type = {
			pwd: {
				func: checkStr,
				agr: {
					who: intl.get(MODULE, 6) /*_i18n:密码*/,
					min: 6,
					max: 32,
					type: 'english'
				}
	...
	dialDetect = async () => {
		...
		common
			.fetchApi([{ opcode: 'WANWIDGET_WAN_LINKSTATE_GET' }])
			.then(resp => {
	...
	render() {
		...
		return (
			<div className="setPassword">
				...
				<p className="ui-tips guide-tip">
					{intl.get(
						MODULE,
						8
					) /*_i18n:管理密码是进入路由器管理页面的凭证*/}
				</p>
				<Form style={{ margin: '24px auto', width: 260 }}>
					...
					>
						<Input
							placeholder={
								intl.get(MODULE, 10) /*_i18n:请设置密码*/
							}
							value={pwd}
					...
					>
						<Input
							placeholder={
								intl.get(MODULE, 12) /*_i18n:请确认密码*/
							}
							value={surePwd}
					...
					<FormItem label="#" labelStyle={{ width: 0 }}>
						<Button
