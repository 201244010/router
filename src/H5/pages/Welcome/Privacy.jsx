...
export default function Privacy(props) {
    return (
        <div className='sm-agreement'>
            ...
            <ul>
                <li>{intl.get(MODULE, 10)/*_i18n:设备应用信息：设备的应用相关信息。例如，安装的应用列表、应用使用记录（下载、安装、更新、卸载）、应用版本、耗电量、消耗流量、占用空间等。*/}</li>
                <li>{intl.get(MODULE, 11)/*_i18n:设备位置信息：设备位置相关的各类信息。例如，国家代码、城市代码、经纬度信息等，设备主要通过GPS、无线局域网或信号塔来确定您设备的大致位置，收集、维护和使用位置数据便于我们提供和改进平台服务。*/}</li>
                <li>{intl.get(MODULE, 12)/*_i18n:设备网络信息：设备使用的网络相关信息。例如，连接的网络类型、网络运营商、MAC地址、路由器信号强度、网络带宽等数据，此类信息有助于我们更好地为您提供网络服务。*/}</li>
                <li>{intl.get(MODULE, 13)/*_i18n:设备打印信息：设备打印机（部分设备无打印机）的相关信息。例如，设备的打印机序列号、型号、版本、打印距离、切刀使用次数、打印头过热报警、打印单数等，收集此类信息的目的在于改善硬件设备的质量以及提供对应的服务。*/}</li>
                <li>{intl.get(MODULE, 14)/*_i18n:设备活跃信息：与您是否使用设备相关信息。例如，当天您的设备处于开机状态，我们会把这台设备标志为活跃。*/}</li>
                <li>{intl.get(MODULE, 67)/*_i18n:设备使用信息：与您使用设备的频率等相关数据。例如，设备CPU占用率、电池温度、开关机情况、摄像头被调用次数等，收集此类信息的目的在于改善或优化下一代设备的硬件配置。*/}</li>
                <li>{intl.get(MODULE, 68)/*_i18n:设备硬件信息：设备相关的硬件信息。例如，设备剩余存储空间、系统版本、MAC地址等。*/}</li>
                <li>{intl.get(MODULE, 69)/*_i18n:外置设备信息：与您设备连接的第三方设备数据。例如，台式设备通过USB或蓝牙接入的键盘、鼠标等外界硬件信息。*/}</li>
                <li>{intl.get(MODULE, 70)/*_i18n:设备流量信息：设备日常使用的流量信息。例如，系统、第三方APP消耗的SIM卡或WIFI流量，收集此类信息的目的在于统计各机具日常使用的流量，便于我们提供和改进平台服务。*/}</li>
                <li>{intl.get(MODULE, 71)/*_i18n:设备其他信息：设备一些系统配置开关信息。例如，设备的开发者选项、个性化设置。*/}</li>
            </ul>
